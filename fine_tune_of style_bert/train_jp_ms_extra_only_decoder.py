...
def run():
    ...
    if getattr(hps.train, "freeze_decoder", False):
        ...
        for param in net_g.dec.parameters():
            param.requires_grad = False
    # === HIROAKI様の意図（Decoder以外を完全凍結）を反映する修正 ===
    # 一旦、Generatorの全パラメータの勾配更新を無効にします
    for param in net_g.parameters():
        param.requires_grad = False
    
    # Decoder（声質を司る部分）のみを学習対象として有効化します
    # これにより、Text Encoder（イントネーション）や Duration Predictor（速さ）が固定されます
    for param in net_g.dec.parameters():
        param.requires_grad = True
        
    logger.info(">>> 韻律を保護するため、Decoder以外の全パラメータを凍結しました。")
    # ========================================================
...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
            ...
            with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
                # === HIROAKI様の意図に基づいた Loss 構成の変更 ===
                # 1. 韻律に関連する Loss を実質的に無効化（0倍）します
                loss_dur = torch.sum(l_length.float()) * 0.0  # 速さ・リズムの強制を停止
                loss_kl = kl_loss(z_p, logs_q, m_p, logs_p, z_mask) * 0.0  # 潜在空間の強制歪みを防止

                # 2. 声質の核となる Mel Loss を主役に据えます
                loss_mel = F.l1_loss(y_mel, y_hat_mel) * hps.train.c_mel
            
                # 3. 補助的な Feature Matching Loss (0.5倍)
                loss_fm = feature_loss(fmap_r, fmap_g) * 0.5

                # 4. GAN Loss（初期段階では影響を極小化 0.1倍、またはOFF）
                loss_gen, losses_gen = generator_loss(y_d_hat_g)
                loss_gen = loss_gen * 0.1
                loss_gen_all = loss_mel + loss_fm + loss_gen + loss_dur + loss_kl
                if net_dur_disc is not None:
