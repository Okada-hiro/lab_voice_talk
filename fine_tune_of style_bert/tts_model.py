...
class TTSModel:
    ...
    def load(self) -> None:
        """
        音声合成モデルをデバイスにロードする。LoRA対応・バグ修正版。
        """
        ...
        if not self.is_onnx_model:
            ...
            # 1. Pydanticモデルを辞書に変換（Mappingエラー対策）
            model_params = self.hyper_parameters.model.model_dump()

            # 2. モデル本体を構築
            if self.hyper_parameters.version.endswith("JP-Extra"):
                self.net_g = SynthesizerTrnJPExtra(
                    len(SYMBOLS),
                    self.hyper_parameters.data.filter_length // 2 + 1,
                    self.hyper_parameters.train.segment_size // self.hyper_parameters.data.hop_length,
                    n_speakers=self.hyper_parameters.data.n_speakers,
                    **model_params,
                ).to(self.device)
            else:
            ...
            # 3. 学習時と同じターゲットに LoRA を適用
            lora_config = LoraConfig(
                r=4, lora_alpha=8,
                target_modules=["bert_proj", "ja_bert_proj", "en_bert_proj", "style_proj", "conv_q", "conv_k", "conv_v", "conv_o", "cond"],
                lora_dropout=0.05, bias="none"
            )
            self.net_g = get_peft_model(self.net_g, lora_config)

            # 4. 名前の不一致（Prefix）を補正してロード
            state_dict = safetensors.torch.load_file(str(self.model_path), device=self.device)
            fixed_state_dict = {}
            for k, v in state_dict.items():
                # キーに base_model がなければ付与、あればそのまま
                new_key = k if k.startswith("base_model.") else f"base_model.model.{k}"
                fixed_state_dict[new_key] = v

            # strict=False にして Posterior Encoder (enc_q) の欠落を許容する
            self.net_g.load_state_dict(fixed_state_dict, strict=False)
            self.net_g.eval()

            logger.info(f'LoRA model loaded with prefix correction from {self.model_path} ({time.time() - start_time:.2f}s)')
        # ONNX 推論時
        else:
