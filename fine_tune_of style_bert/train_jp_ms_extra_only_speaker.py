...
def run():
    ...
    if getattr(hps.train, "freeze_decoder", False):
        ...
        for param in net_g.dec.parameters():
            param.requires_grad = False
    # === 修正箇所1：学習対象を 'emb_g' + 'Decoder全体' に設定 ===
    # 全てのパラメータを一旦凍結します
    ...
    for name, param in net_g.named_parameters():
        if "emb_g" in name:
            ...
            logger.info(f">>> 話者ベクトルを解放: {name}")

    
    logger.info(">>> 韻律構造（Encoder/Flow/Duration）は引き続き固定（Frozen）されています。")
    # ==========================================================
    ...
    for batch_idx, (
        ...
        with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
            ...
            with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
                ...
                loss_fm = feature_loss(fmap_r, fmap_g) * 0.1
