...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
            ...
            # Discriminator
            # ===================================================================================
            # 【修正版】 Discriminatorの更新処理全体を条件分岐の中に入れる
            # ===================================================================================
            if False:  # GAN無効化 (Trueにすれば元に戻る)
                # --- 元のDiscriminator更新処理 ---
                ...
                # Main Discriminator Optimizer Step
                optim_d.zero_grad()
                ...
                # Duration Discriminator用のダミー
                loss_dur_disc_all = torch.tensor(0.0, device=y.device)
                losses_dur_disc_r = []
                losses_dur_disc_g = []
                grad_norm_dur = 0.0
                
                # WavLM用のダミー
                loss_slm = torch.tensor(0.0, device=y.device)
                grad_norm_wd = 0.0
        ...
        with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
            ...
            if False: 
                ...
                # 計算せずにダミーの値を入れる（これで計算コスト0）
                y_d_hat_g = None
                fmap_r = None
                fmap_g = None
            # Duration Discriminator も同様にスキップ
            if False and (net_dur_disc is not None): # ここもFalseにする
                _, y_dur_hat_g = net_dur_disc(hidden_x, x_mask, logw_, logw, g)
            else:
                y_dur_hat_g = None

            # WavLM も同様
            if False and (net_wd is not None):
                 loss_lm = wl(y.detach().squeeze(1), y_hat.squeeze(1)).mean()
                 loss_lm_gen = wl.generator(y_hat.squeeze(1))
            else:
                 loss_lm = torch.tensor(0.0, device=y.device)
                 loss_lm_gen = torch.tensor(0.0, device=y.device)
            ...
            with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
                ...
                losses_gen = [] # 追加: ログ用変数の定義
                loss_dur_gen = torch.tensor(0.0, device=y.device)
                losses_dur_gen = [] # 追加: ログ用変数の定義
