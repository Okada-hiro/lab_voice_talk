...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    # 保存先ディレクトリの作成: Data/モデル名/models/eval_wavs
    save_dir = os.path.join(hps.model_dir, "eval_wavs")
    os.makedirs(save_dir, exist_ok=True)

    print()
    logger.info(f"Evaluating and saving wavs to {save_dir} ...")
    with torch.no_grad():
        for batch_idx, (
            x,
            x_lengths,
            ...
            for use_sdp in [True, False]:
                ...
                # TensorBoard用
                key_name = f"gen/audio_{batch_idx}_{use_sdp}"
                audio_dict.update({key_name: y_hat[0, :, : y_hat_lengths[0]]})

                # ★wav保存 (Text -> Audio)
                # ファイル名: step_XXXX_batch_0_gen_True.wav
                audio_numpy = y_hat[0, 0, : y_hat_lengths[0]].cpu().float().numpy()
                file_name = f"step_{global_step}_batch_{batch_idx}_gen_sdp{use_sdp}.wav"
                scipy.io.wavfile.write(
                    os.path.join(save_dir, file_name),
                    hps.data.sampling_rate,
                    audio_numpy
                )
            # --- 2. 再構成 (Posterior経由: Spec -> Audio) ---
            # 話者埋め込み
            g = None
            if generator.module.n_speakers > 0:
                g = generator.module.emb_g(speakers).unsqueeze(-1)
            
            # Posterior Encoder -> Decoder
            z_post, m_q, logs_q, _ = generator.module.enc_q(spec, spec_lengths, g=g)
            y_hat_post = generator.module.dec(z_post, g=g)
            
            # TensorBoard用
            audio_dict.update(
                {
                    f"rec/audio_{batch_idx}_posterior": y_hat_post[0, :, : y_lengths[0]]
                }
            )

            # ★wav保存 (Spec -> Audio: Reconstruction)
            # ファイル名: step_XXXX_batch_0_rec.wav
            audio_numpy_post = y_hat_post[0, 0, : y_lengths[0]].cpu().float().numpy()
            file_name_post = f"step_{global_step}_batch_{batch_idx}_rec.wav"
            scipy.io.wavfile.write(
                os.path.join(save_dir, file_name_post),
                hps.data.sampling_rate,
                audio_numpy_post
            )
            ...
            # 正解データも保存（比較用）
            audio_numpy_gt = y[0, 0, : y_lengths[0]].cpu().float().numpy()
            file_name_gt = f"step_{global_step}_batch_{batch_idx}_gt.wav"
            scipy.io.wavfile.write(
                os.path.join(save_dir, file_name_gt),
                hps.data.sampling_rate,
                audio_numpy_gt
            )
