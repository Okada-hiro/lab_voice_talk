...
def apply_lora_to_model(net_g, lora_config, rank):
    """
    Apply LoRA to the generator model.
    Returns the LoRA-wrapped model.
    """
    if rank == 0:
        logger.info("Applying LoRA to the generator model...")
        logger.info(f"LoRA config: r={lora_config.r}, alpha={lora_config.lora_alpha}")
        logger.info(f"Target modules: {lora_config.target_modules}")
    
    # Apply LoRA using PEFT
    net_g_lora = get_peft_model(net_g, lora_config)
    
    if rank == 0:
        # Print trainable parameters info
        net_g_lora.print_trainable_parameters()
...
def run():
    ...
    parser.add_argument(
        ...
        "--model",
        type=str,
        help="データセットフォルダ路径,请注意,数据不再默认放在/logs文件夹下。如果需要用命令行配置,请声明相对于根目录的路径",
    ...
    parser.add_argument(
        "--use_lora",
        action="store_true",
        help="Use LoRA for fine-tuning the generator model.",
    )
    parser.add_argument(
        "--lora_r",
        type=int,
    ...
    if utils.is_resuming(model_dir):
        ...
        try:
            _, _, _, epoch_str = utils.checkpoints.load_checkpoint(
                utils.checkpoints.get_latest_checkpoint_path(model_dir, "G_*.pth"),
                net_g,
                None,  # Don't load optimizer yet
                skip_optimizer=True,
            )
            global_step = int(
                utils.get_steps(
            ...
            logger.info(
                f"Loaded checkpoint. Current epoch is {epoch_str}, global step is {global_step}"
            )
        except Exception as e:
            logger.warning(f"Could not load checkpoint: {e}")
    
    if not is_pretrained_loaded:
        # Try to load pretrained safetensors
        try:
            _ = utils.safetensors.load_safetensors(
                os.path.join(model_dir, "G_0.safetensors"), net_g
            )
            is_pretrained_loaded = True
            logger.info("Loaded pretrained model from safetensors.")
        except Exception as e:
            logger.warning(f"Could not load pretrained safetensors: {e}")
    ...
    using_lora = args.use_lora
    if using_lora:
        lora_config = LoraConfig(
            r=args.lora_r,
            lora_alpha=args.lora_alpha,
            target_modules=["bert_proj", "ja_bert_proj", "en_bert_proj", "style_proj", "conv_q", "conv_k", "conv_v", "conv_o", "cond"],
            lora_dropout=args.lora_dropout,
            bias="none"
        )
        net_g = apply_lora_to_model(net_g, lora_config, rank)
    
    # Handle freezing options
    if getattr(hps.train, "freeze_JP_bert", False) and not using_lora:
        logger.info("Freezing (JP) bert encoder !!!")
        for param in net_g.enc_p.bert_proj.parameters():
            param.requires_grad = False
    if getattr(hps.train, "freeze_style", False) and not using_lora:
    ...
    # NOW load discriminator and other checkpoints if resuming
    if utils.is_resuming(model_dir):
        if net_dur_disc is not None:
            try:
                _, _, dur_resume_lr, _ = utils.checkpoints.load_checkpoint(
        ...
        try:
            _, optim_g, g_resume_lr, _ = utils.checkpoints.load_checkpoint(
                utils.checkpoints.get_latest_checkpoint_path(model_dir, "G_*.pth"),
            ...
            logger.warning(f"Could not load generator optimizer: {e}")
        
        # Load discriminator
        try:
            _, optim_d, d_resume_lr, _ = utils.checkpoints.load_checkpoint(
        ...
        # Load discriminator pretrained weights
        try:
            _ = utils.safetensors.load_safetensors(
            ...
            logger.info("Loaded discriminator pretrained models.")
        except Exception as e:
            logger.warning(f"Could not load discriminator pretrained models: {e}")
...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        # Get base model for MAS noise scale check
        base_model = net_g.module.base_model if hasattr(net_g.module, 'base_model') else net_g.module
        
        if base_model.use_noise_scaled_mas:
            current_mas_noise_scale = (
                base_model.mas_noise_scale_initial
                - base_model.noise_scale_delta * global_step
            )
            base_model.current_mas_noise_scale = max(current_mas_noise_scale, 0.0)
