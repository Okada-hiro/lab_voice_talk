...
async def process_voice_pipeline(audio_float32_np, websocket: WebSocket, chat_history: list):
    ...
    if NEXT_AUDIO_IS_REGISTRATION:
        ...
        logger.info(f"[PIPELINE_TIMING] register_new_speaker_ms={reg_ms:.1f}")
        NEXT_AUDIO_IS_REGISTRATION = False 
        ...
        auth_ms = (time.perf_counter() - auth_t0) * 1000
        logger.info(f"[PIPELINE_TIMING] identify_speaker_ms={auth_ms:.1f}")
    ...
    try:
        ...
        asr_transcribe_ms = (time.perf_counter() - asr_t0) * 1000
        words_t0 = time.perf_counter()
        
        text = "".join([s[2] for s in GLOBAL_ASR_MODEL_INSTANCE.ts_words(segments)])
        asr_words_ms = (time.perf_counter() - words_t0) * 1000
        logger.info(
            f"[PIPELINE_TIMING] asr_transcribe_ms={asr_transcribe_ms:.1f} "
            f"asr_words_ms={asr_words_ms:.1f}"
        )
        ...
        # 4. LLM & TTS ストリーミング
        # ---------------------------
        llm_tts_t0 = time.perf_counter()
        await handle_llm_tts(text_with_context, websocket, chat_history)
        llm_tts_ms = (time.perf_counter() - llm_tts_t0) * 1000
        pipeline_total_ms = (time.perf_counter() - pipeline_t0) * 1000
        logger.info(
            f"[PIPELINE_TIMING] llm_tts_ms={llm_tts_ms:.1f} total_pipeline_ms={pipeline_total_ms:.1f}"
        )
...
async def handle_llm_tts(text_for_llm: str, websocket: WebSocket, chat_history: list):
    ...
    async def audio_sender_worker():
        nonlocal sent_count, send_ms_sum
        while True:
            ...
            try:
                ...
                idx, full_wav_bytes, enqueue_ts = item
                ...
                e2e_ms = (time.perf_counter() - enqueue_ts) * 1000
                sent_count += 1
                send_ms_sum += send_ms
                logger.info(
                    f"🚀 Streamed audio {idx} (Total: {total_len} bytes) "
                    f"[LLM_TTS_TIMING] send_ms={send_ms:.1f} e2e_ms={e2e_ms:.1f}"
                )
            finally:
    ...
    try:
        llm_stream_t0 = time.perf_counter()
        for chunk in iterator:
            if first_llm_chunk_t is None:
                first_llm_chunk_t = time.perf_counter()
                first_llm_chunk_ms = (first_llm_chunk_t - llm_stream_t0) * 1000
                logger.info(f"[LLM_TTS_TIMING] llm_first_chunk_ms={first_llm_chunk_ms:.1f}")
            text_buffer += chunk
            full_answer += chunk
            ...
            if len(sentences) > 1:
                for sent in sentences[:-1]:
                    if sent.strip():
                        ...
                        await text_queue.put((sentence_count, sent, time.perf_counter()))
                text_buffer = sentences[-1]
        
        if text_buffer.strip():
            sentence_count += 1
            await websocket.send_json({"status": "reply_chunk", "text_chunk": text_buffer})
            await text_queue.put((sentence_count, text_buffer, time.perf_counter()))
        ...
        total_ms = (time.perf_counter() - llm_tts_t0) * 1000
        avg_tts_ms = (tts_ms_sum / tts_count) if tts_count else 0.0
        avg_send_ms = (send_ms_sum / sent_count) if sent_count else 0.0
        logger.info(
            f"[LLM_TTS_TIMING] total_ms={total_ms:.1f} "
            f"tts_count={tts_count} avg_tts_ms={avg_tts_ms:.1f} "
            f"sent_count={sent_count} avg_send_ms={avg_send_ms:.1f}"
        )
